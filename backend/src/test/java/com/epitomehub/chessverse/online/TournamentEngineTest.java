...
class TournamentEngineTest {
    ...
    @BeforeEach
    void schema() {
        jdbc.execute("create table if not exists chess_tournament(id uuid primary key,name varchar(100),description varchar(300),time_control_minutes int,capacity int,starts_at timestamp with time zone,ends_at timestamp with time zone,status varchar(16),current_round int default 0,champion_id uuid,entry_coins int default 100)");
        ...
        jdbc.update("merge into chess_tournament(id,name,description,time_control_minutes,capacity,starts_at,ends_at,status,current_round,entry_coins) key(id) values(?,?,?,?,?,?,?,?,?,?)",
    ...
    void createsBracketAndDeclaresChampionFromAuthoritativeMatch() throws Exception {
        ...
        assertEquals(400L, jdbc.queryForObject("select coin_balance from player_wallet where player_id=?",Long.class,firstId));
        assertEquals(400L, jdbc.queryForObject("select coin_balance from player_wallet where player_id=?",Long.class,secondId));
        ...
        assertEquals(600L, jdbc.queryForObject("select coin_balance from player_wallet where player_id=?",Long.class,replay.whitePlayerId));
        assertEquals(400L, jdbc.queryForObject("select coin_balance from player_wallet where player_id=?",Long.class,replay.blackPlayerId));
    }
