...
class AcademyPaymentGateway {
    ...
    AcademyPaymentGateway(ObjectMapper json,
            @Value("${chessverse.academy.payments.enabled:false}") boolean enabled,
            @Value("${chessverse.academy.payments.key-id:}") String keyId,
            @Value("${chessverse.academy.payments.key-secret:}") String keySecret,
            @Value("${chessverse.academy.payments.webhook-secret:}") String webhookSecret) {
    ...
    boolean available() {
        return enabled && keyId.startsWith("rzp_live_") && !keySecret.isBlank();
    }
    ...
    ProviderOrder createOrder(String receipt, long amountMinor, String currency) {
        ...
        try {
            String body = json.writeValueAsString(Map.of(
                    ...
                    "currency", currency,
                    "receipt", receipt,
                    "notes", Map.of("purpose", "ChessVerseAI academy license")));
        ...
        try {
            ...
            if (!paymentId.equals(payment.path("id").asText())
                ...
                throw new ResponseStatusException(HttpStatus.CONFLICT,
                        "Payment is not captured for this exact order. Academy was not activated.");
            }
    ...
    private ResponseStatusException unavailable() {
        return new ResponseStatusException(HttpStatus.BAD_GATEWAY,
                "Secure web checkout is temporarily unavailable. Check payment status before retrying.");
    }
