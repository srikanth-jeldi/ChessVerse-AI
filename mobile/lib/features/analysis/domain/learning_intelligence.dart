...
class LearningIntelligence {
  ...
  factory LearningIntelligence.fromGames(
    ...
    final List<SavedGameRecord> current = games
        .where(
          (SavedGameRecord game) => !game.playedAt.toUtc().isBefore(
            anchor.subtract(const Duration(days: 7)),
          ),
        )
        .toList(growable: false);
    final List<SavedGameRecord> previous = games
    ...
    final int currentMistakes = _mistakeCount(current);
    final int previousMistakes = _mistakeCount(previous);
    final List<SavedGameRecord> chronological = games
        .take(12)
        .toList()
        .reversed
        .toList();
    final List<ProgressTrendPoint> trend = <ProgressTrendPoint>[
      for (int index = 0; index < chronological.length; index++)
        ProgressTrendPoint(
          'G${index + 1}',
          _gameAccuracy(chronological[index]),
        ),
    ];
    ...
    return LearningIntelligence(
      ...
      weekly: WeeklyProgressReport(
        ...
        reviewedMoves: current.fold<int>(
          0,
          (int total, SavedGameRecord game) => total + game.moveReviews.length,
        ),
        averageAccuracy: currentAccuracy,
        accuracyChange: previous.isEmpty
            ? 0
            : currentAccuracy - previousAccuracy,
        mistakes: currentMistakes,
        mistakeChange: previous.isEmpty
            ? 0
            : currentMistakes - previousMistakes,
...
int _mistakeCount(List<SavedGameRecord> games) => games.fold<int>(
  0,
