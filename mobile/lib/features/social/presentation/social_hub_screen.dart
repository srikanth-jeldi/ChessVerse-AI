...
class _SocialHubScreenState extends State<SocialHubScreen> {
  ...
  Future<void> _createClubTournament() async {
    final ClubDto? club = _community?.clubs
        .where((item) => item.joined)
        .firstOrNull;
    if (club == null || _session == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Join a club before creating its private tournament.'),
        ),
      );
      return;
    }
    ...
    await _communityAct(
      () => _communityApi.createClubTournament(
        _session!.token,
        club.id,
        name: tournamentName,
        description:
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                  colors: <Color>[Color(0xFF0C2536), Color(0xFF071725)],
                ),
  ...
  Future<void> _act(
    Future<SocialHubDto> Function() action, {
  ...
  Future<void> _challenge(SocialPlayerDto friend) async {
    ...
    try {
      final SocialChallengeDto value = await _api.challenge(
        _session!.token,
        friend.playerId,
        choice,
      );
  ...
  Future<void> _accept(SocialChallengeDto challenge) async {
    ...
    try {
      final OnlineMatchDto match = await _api.acceptChallenge(
        _session!.token,
        challenge.id,
      );
  ...
  Future<void> _decline(SocialChallengeDto challenge) async {
    if (_session == null) return;
    try {
      final SocialHubDto hub = await _api.declineChallenge(
        _session!.token,
        challenge.id,
      );
      if (!mounted) return;
  ...
  Future<void> _notificationAction(PlayerNotificationDto item) async {
    ...
    if (action == 'MATCH' && item.actionId != null && _session != null) {
      try {
        final match = await const OnlineMatchApi().getMatch(
          _session!.token,
          item.actionId!,
        );
        if (mounted) widget.onOpenMatch?.call(match);
  ...
  Widget build(BuildContext context) => Scaffold(
      ...
      actions: <Widget>[
        NotificationBellButton(
          onPressed: () => Navigator.of(context).push(
            MaterialPageRoute<void>(
              builder: (_) =>
                  NotificationCenterScreen(onAction: _notificationAction),
            ),
          ),
        ),
        if (_section == 0)
    ...
    body: _busy
                    ...
                    _Section(
                      ...
                      children: _hub!.outgoing
                          .map(
                            (player) => _PlayerCard(
                              player: player,
                              primaryLabel: player.relationship == 'DECLINED'
                                  ? 'Rejected'
                                  : 'Requested',
        ...
        : _CommunitySection(
            ...
            onRefresh: _refresh,
            onClub: (club) => _communityAct(
              () => _communityApi.club(_session!.token, club.id, !club.joined),
            ),
              ...
              child: InkWell(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: active
                        ? const <BoxShadow>[
                            BoxShadow(color: Color(0x9948E5D1), blurRadius: 17),
                          ]
                        : null,
                  ),
                  child: Row(
...
class _CommunitySection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<SocialPlayerDto> chatPlayers = <SocialPlayerDto>[
      ...community.conversations.map(
        (ConversationDto conversation) => SocialPlayerDto(
          connectionId: '',
      ...
      ...friends.where(
        (SocialPlayerDto friend) => !community.conversations.any(
          (ConversationDto item) => item.playerId == friend.playerId,
        ),
      ),
    ];
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  if (section == 1) ...<Widget>[
                    FilledButton.icon(
                      key: const ValueKey<String>('create-club-tournament'),
                      onPressed: onCreateClubTournament,
                      icon: const Icon(Icons.add_rounded),
                      label: const Text('CREATE PRIVATE CLUB TOURNAMENT'),
                    ),
                ...
                children: <Widget>[
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: const Color(0xFF51DFC9)),
                    ),
    ...
    child: Column(
      ...
      children: <Widget>[
        const CircleAvatar(
          ...
          child: Icon(Icons.forum_outlined, size: 52, color: Color(0xFF55E4CF)),
        ),
...
class _ChatScreenState extends State<_ChatScreen> {
  ...
  Future<MessageDto> _decryptMessage(MessageDto message) async {
    ...
    final String plaintext = await _e2ee.decrypt(
      message.body,
      mine: message.mine,
    );
  ...
  Future<void> _load({bool silent = false}) async {
    try {
      final List<MessageDto> encrypted = await widget.api.messages(
        widget.token,
        widget.friend.playerId,
      );
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (mounted) {
        setState(
          () => _messages = _messages
              .map((item) => item.id == pending.id ? m : item)
              .toList(),
        );
      }
      _scrollToLatest();
  ...
  Future<void> _react(MessageDto message, String? emoji) async {
    try {
      final MessageDto updated = await widget.api.react(
        widget.token,
        message.id,
        emoji,
      );
  ...
  Future<void> _deleteMessage(MessageDto message, bool forEveryone) async {
    try {
      await widget.api.deleteMessage(
        widget.token,
        message.id,
              ...
              children: <Widget>[
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        _replyingTo!.mine ? 'You' : widget.friend.displayName,
                        style: const TextStyle(
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, int index) => ActionChip(
                ...
                onPressed: () => widget.onFallbackSticker?.call(
                  widget.fallbackStickers[index],
                ),
              ),
            ),
          ),
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          constraints: BoxConstraints(
            maxWidth: width > 800 ? 430 : width * .78,
          ),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (visibleBody.isNotEmpty || mediaUrl == null)
                      Wrap(
                    ...
                    if (message.reactions.isNotEmpty) ...<Widget>[
                      ...
                      Wrap(
                        ...
                        children: message.reactions
                            .map(
                              (MessageReactionDto reaction) => Container(
...
class _ReceiptTicks extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color color = seen
        ? const Color(0xFF20DDF2)
...
class _ChatAttachmentState extends State<_ChatAttachment> {
  ...
  Future<List<int>> _loadBytes() async {
    final List<int> bytes = await widget.api.attachmentBytes(
      widget.token,
      widget.message.id,
    );
...
class _ChatBubblePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect body = Rect.fromLTRB(
      outgoing ? 0 : tail,
      0,
      outgoing ? size.width - tail : size.width,
      size.height,
    );
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      image: const DecorationImage(
        image: AssetImage('assets/backgrounds/home-friends-hero-v1.webp'),
        fit: BoxFit.cover,
        alignment: Alignment.centerRight,
        opacity: .38,
      ),
        ...
        final Widget identity = const Expanded(
          child: Column(
            ...
            children: <Widget>[
              ...
              SizedBox(height: 5),
              Text(
                'Friends, private challenges and live rivals',
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        const SizedBox(height: 6),
        const Text(
          'Add friends by their ChessVerseAI username.',
      ...
      children: <Widget>[
        ...
        Column(
          ...
          children: <Widget>[
            ...
            Text(
              label,
              style: const TextStyle(color: Color(0xFF91A6B3), fontSize: 10),
            ),
          ],
        ),
      ],
