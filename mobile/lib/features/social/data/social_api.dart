...
class SocialPlayerDto {
  ...
  factory SocialPlayerDto.fromJson(Map<String, dynamic> json) =>
      SocialPlayerDto(
        connectionId: json['connectionId'] as String? ?? '',
        playerId: json['playerId'] as String? ?? '',
        username: json['username'] as String? ?? '',
        displayName: json['displayName'] as String? ?? 'ChessVerseAI Player',
        photoUrl: _absolutePhotoUrl(json['photoUrl'] as String?),
...
class SocialChallengeDto {
  ...
  factory SocialChallengeDto.fromJson(Map<String, dynamic> json) =>
      SocialChallengeDto(
        id: json['id'] as String? ?? '',
        opponentName: json['opponentName'] as String? ?? 'Player',
...
class SocialApi {
  const SocialApi();
  Future<SocialHubDto> load(String token) async =>
      SocialHubDto.fromJson(await _request(token, 'GET', '/api/v1/social'));
  Future<SocialHubDto> addFriend(String token, String username) async =>
      SocialHubDto.fromJson(await _request(
          token, 'POST', '/api/v1/social/friends',
          body: <String, Object?>{'username': username}));
  Future<SocialHubDto> respond(
          String token, String connectionId, bool accept) async =>
      SocialHubDto.fromJson(await _request(
          token, 'PUT', '/api/v1/social/friends/$connectionId?accept=$accept'));
  Future<SocialHubDto> remove(String token, String friendId) async =>
      SocialHubDto.fromJson(
          await _request(token, 'DELETE', '/api/v1/social/friends/$friendId'));
  Future<SocialChallengeDto> challenge(
          String token, String friendId, int minutes) async =>
      SocialChallengeDto.fromJson(await _request(
          token, 'POST', '/api/v1/social/challenges', body: <String, Object?>{
        'friendId': friendId,
        'timeControlMinutes': minutes
      }));
  Future<OnlineMatchDto> acceptChallenge(String token, String id) async =>
      OnlineMatchDto.fromJson(await _request(
          token, 'POST', '/api/v1/social/challenges/$id/accept'));
  Future<SocialHubDto> declineChallenge(String token, String id) async =>
      SocialHubDto.fromJson(await _request(
          token, 'POST', '/api/v1/social/challenges/$id/decline'));
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        throw SocialException(
            error['message'] as String? ?? 'Social request failed.');
      }
      return decoded is Map<String, dynamic> ? decoded : <String, dynamic>{};
    } on SocialException {
      rethrow;
    } on TimeoutException {
      throw const SocialException('Request timed out. Try again.');
    } catch (_) {
      throw const SocialException('Cannot reach ChessVerseAI social services.');
    }
  }
}
