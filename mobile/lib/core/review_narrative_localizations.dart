...
String reviewNarrativeText(
  String key,
  String code, [
  Map<String, String> values = const {},
]) {
  final index = reviewNarrativeKeys.indexOf(key);
  if (index < 0) throw ArgumentError.value(key, 'key');
  var result =
      reviewNarrativeTranslations[AppLanguageController.resolveCode(
        code,
      )]![index];
...
bool supportsReviewNarrative(String value) {
  ...
  final metadata = RegExp(
    r'^(.+?) • (.+?) • (\d+) book plies • first deviation ply (\d+)$',
  ).firstMatch(value);
...
String localizeReviewNarrative(String value, String code) {
  ...
  if (metadata != null && supportsReviewNarrative(value)) {
    return '${metadata.group(1)} • ${localizeReviewNarrative(metadata.group(2)!, language)} • ${text('bookMetadata', {'count': metadata.group(3)!, 'ply': metadata.group(4)!})}';
  }
  ...
  if (wrapper != null && supportsReviewNarrative(wrapper.group(4)!)) {
    return '${text('move', {'count': wrapper.group(1)!})}: ${wrapper.group(2)} ${wrapper.group(3)} ${localizeReviewNarrative(wrapper.group(4)!, language)}';
  }
  final turn = RegExp(
    r'^([a-h][1-8](?: ?x ?)?[a-h][1-8][qrbn]?(?: e\.p\.)?|O-O(?:-O)?) ([•—]) (.+)$',
  ).firstMatch(value);
...
const _cloudExtras = <String, String>{
  ...
  'te': 'ఇది స్టాక్‌ఫిష్ అత్యంత బలమైన కొనసాగింపుతో సరిపోలింది.|{move} ఎత్తు {count} సెంటిపాన్‌ల మేర బలంగా ఉంది.',
  'hi': 'यह स्टॉकफिश के सबसे मजबूत क्रम से मेल खाती थी।|{move} चाल {count} सेंटीपॉन अधिक मजबूत थी।',
  'ta': 'இது ஸ்டாக்ஃபிஷின் வலுவான தொடர்ச்சியுடன் பொருந்தியது.|{move} நகர்வு {count} சென்டிபான் அளவு வலுவானது.',
  'kn': 'ಇದು ಸ್ಟಾಕ್‌ಫಿಷ್‌ನ ಅತ್ಯಂತ ಬಲವಾದ ಮುಂದುವರಿಕೆಗೆ ಹೊಂದಿತ್ತು.|{move} ನಡೆ {count} ಸೆಂಟಿಪಾನ್‌ಗಳಷ್ಟು ಹೆಚ್ಚು ಬಲವಾಗಿತ್ತು.',
  'ml': 'ഇത് സ്റ്റോക്ക്ഫിഷിന്റെ ഏറ്റവും ശക്തമായ തുടർച്ചയുമായി പൊരുത്തപ്പെട്ടു.|{move} നീക്കം {count} സെന്റിപോൺ കൂടുതൽ ശക്തമായിരുന്നു.',
  'mr': 'ही स्टॉकफिशच्या सर्वात मजबूत क्रमाशी जुळली.|{move} चाल {count} सेंटिपॉनने अधिक मजबूत होती.',
  'bn': 'এটি স্টকফিশের সবচেয়ে শক্তিশালী ধারাবাহিকতার সঙ্গে মিলেছিল।|{move} চালটি {count} সেন্টিপন বেশি শক্তিশালী ছিল।',
  'gu': 'આ સ્ટોકફિશના સૌથી મજબૂત ક્રમ સાથે મેળ ખાતી હતી.|{move} ચાલ {count} સેન્ટિપોન જેટલી વધુ મજબૂત હતી.',
  'pa': 'ਇਹ ਸਟਾਕਫਿਸ਼ ਦੇ ਸਭ ਤੋਂ ਮਜ਼ਬੂਤ ਕ੍ਰਮ ਨਾਲ ਮੇਲ ਖਾਂਦੀ ਸੀ।|{move} ਚਾਲ {count} ਸੈਂਟੀਪਾਨ ਵਧੇਰੇ ਮਜ਼ਬੂਤ ਸੀ।',
  'ur': 'یہ اسٹاک فش کے مضبوط ترین تسلسل سے مطابقت رکھتی تھی۔|{move} چال {count} سینٹی پون زیادہ مضبوط تھی۔',
  'ar': 'طابقت هذه أقوى تكملة لستوكفيش.|كانت {move} أقوى بمقدار {count} سنتيبيدق.',
  'fa': 'این با قوی‌ترین ادامهٔ استاک‌فیش مطابقت داشت.|{move} به اندازهٔ {count} سنتی‌پیاده قوی‌تر بود.',
  'es': 'Esto coincidía con la continuación más fuerte de Stockfish.|{move} era más fuerte por {count} centipeones.',
  'fr': 'Cela correspondait à la suite la plus forte de Stockfish.|{move} était plus fort de {count} centipions.',
  'de': 'Dies entsprach der stärksten Fortsetzung von Stockfish.|{move} war um {count} Zentibauern stärker.',
  'it': 'Questo corrispondeva alla continuazione più forte di Stockfish.|{move} era più forte di {count} centesimi di pedone.',
  'pt': 'Isso correspondia à continuação mais forte do Stockfish.|{move} era mais forte por {count} centipeões.',
  'ru': 'Это совпало с сильнейшим продолжением Stockfish.|{move} было сильнее на {count} сантипешек.',
  'uk': 'Це збіглося з найсильнішим продовженням Stockfish.|{move} було сильніше на {count} сантипішаків.',
  'tr': 'Bu, Stockfish’in en güçlü devam yoluyla eşleşti.|{move}, {count} centipawn daha güçlüydü.',
  ...
  'id': 'Ini cocok dengan kelanjutan terkuat Stockfish.|{move} lebih kuat sebesar {count} centipawn.',
  'ms': 'Ini sepadan dengan sambungan terkuat Stockfish.|{move} lebih kuat sebanyak {count} centipawn.',
  'th': 'ตานี้ตรงกับแนวเดินต่อที่แข็งแกร่งที่สุดของ Stockfish|{move} แข็งแกร่งกว่า {count} เซนติพอน',
  'vi': 'Nước này khớp với diễn biến mạnh nhất của Stockfish.|{move} mạnh hơn {count} centipawn.',
  'pl': 'To odpowiadało najsilniejszej kontynuacji Stockfisha.|{move} było silniejsze o {count} centypionów.',
  'nl': 'Dit kwam overeen met de sterkste voortzetting van Stockfish.|{move} was {count} centipionnen sterker.',
  'sv': 'Detta motsvarade Stockfishs starkaste fortsättning.|{move} var {count} centibönder starkare.',
  'el': 'Αυτό ταίριαζε με την ισχυρότερη συνέχεια του Stockfish.|Το {move} ήταν ισχυρότερο κατά {count} εκατοστά πιονιού.',
  'he': 'זה תאם להמשך החזק ביותר של Stockfish.|{move} היה חזק יותר ב-{count} סנטי-רגלי.',
  'sw': 'Hii ililingana na mwendelezo wenye nguvu zaidi wa Stockfish.|{move} ilikuwa na nguvu zaidi kwa centipawn {count}.',
};

const _fallbackExtras = <String, String>{
  'en': 'You found the strongest continuation.|{move} was more accurate.|Prefer {move}.',
  'te': 'అత్యంత బలమైన కొనసాగింపును కనుగొన్నారు.|{move} మరింత ఖచ్చితమైనది.|{move} ఎత్తుకు ప్రాధాన్యం ఇవ్వండి.',
  'hi': 'आपने सबसे मजबूत क्रम खोज लिया।|{move} अधिक सटीक थी।|{move} को प्राथमिकता दें।',
  'ta': 'வலுவான தொடர்ச்சியைக் கண்டறிந்தீர்கள்.|{move} இன்னும் துல்லியமானது.|{move}க்கு முன்னுரிமை தரவும்.',
  'kn': 'ಅತ್ಯಂತ ಬಲವಾದ ಮುಂದುವರಿಕೆಯನ್ನು ಕಂಡಿರಿ.|{move} ಹೆಚ್ಚು ನಿಖರವಾಗಿತ್ತು.|{move}ಗೆ ಆದ್ಯತೆ ನೀಡಿ.',
  'ml': 'ഏറ്റവും ശക്തമായ തുടർച്ച കണ്ടെത്തി.|{move} കൂടുതൽ കൃത്യമായിരുന്നു.|{move}ന് മുൻഗണന നൽകുക.',
  'mr': 'तुम्ही सर्वात मजबूत क्रम शोधला.|{move} अधिक अचूक होती.|{move} ला प्राधान्य द्या.',
  'bn': 'সবচেয়ে শক্তিশালী ধারাবাহিকতা খুঁজেছেন।|{move} বেশি নির্ভুল ছিল।|{move} বেছে নিন।',
  'gu': 'તમે સૌથી મજબૂત ક્રમ શોધ્યો.|{move} વધુ સચોટ હતી.|{move} ને પ્રાધાન્ય આપો.',
  'pa': 'ਤੁਸੀਂ ਸਭ ਤੋਂ ਮਜ਼ਬੂਤ ਕ੍ਰਮ ਲੱਭਿਆ।|{move} ਵਧੇਰੇ ਸਹੀ ਸੀ।|{move} ਨੂੰ ਪਹਿਲ ਦਿਓ।',
  'ur': 'آپ نے مضبوط ترین تسلسل تلاش کیا۔|{move} زیادہ درست تھی۔|{move} کو ترجیح دیں۔',
  'ar': 'وجدت أقوى تكملة.|كانت {move} أدق.|فضّل {move}.',
  'fa': 'قوی‌ترین ادامه را یافتید.|{move} دقیق‌تر بود.|{move} را ترجیح دهید.',
  'es': 'Encontraste la continuación más fuerte.|{move} era más precisa.|Prefiere {move}.',
  'fr': 'Vous avez trouvé la suite la plus forte.|{move} était plus précis.|Préférez {move}.',
  'de': 'Du hast die stärkste Fortsetzung gefunden.|{move} war genauer.|Bevorzuge {move}.',
  'it': 'Hai trovato la continuazione più forte.|{move} era più precisa.|Preferisci {move}.',
  'pt': 'Você encontrou a continuação mais forte.|{move} era mais preciso.|Prefira {move}.',
  'ru': 'Вы нашли сильнейшее продолжение.|{move} было точнее.|Предпочтите {move}.',
  'uk': 'Ви знайшли найсильніше продовження.|{move} було точніше.|Віддайте перевагу {move}.',
  'tr': 'En güçlü devam yolunu buldunuz.|{move} daha doğruydu.|{move} tercih edin.',
