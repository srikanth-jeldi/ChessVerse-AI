...
void main() {
  test(
    ...
    () async {
      ...
      final _IdentityApi api = _IdentityApi(<String, String>{
        'a': alice,
        'b': bob,
      });
      final E2eeChatService first = E2eeChatService(
        api: api,
      ...
      final E2eeChatService second = E2eeChatService(
        api: api,
      ...
      final E2eeChatService backgroundReceiver = E2eeChatService(
        api: api,
        storage: secondStorage,
      );
      expect(
      ...
      final E2eeChatService restored = E2eeChatService(
        api: api,
        storage: _MemoryStorage(),
      );
      await restored.restore('a', aliceSetup.recoveryKey!);
      expect(
        await restored.decrypt(envelope, mine: true),
        'private chess plan',
      );
    },
  );
  ...
  test(
    'lost local identity can be replaced for new encrypted photos',
    () async {
      const String alice = '11111111-1111-1111-1111-111111111111';
      const String bob = '22222222-2222-2222-2222-222222222222';
      final _IdentityApi api = _IdentityApi(<String, String>{
      ...
      final E2eeChatService bobService = E2eeChatService(
        api: api,
        storage: _MemoryStorage(),
      );
      await bobService.initialize('b', alice);

      final E2eeChatService replacement = E2eeChatService(
        api: api,
        storage: _MemoryStorage(),
      );
      final E2eeSetupResult setup = await replacement.resetIdentity('a', bob);
      expect(setup.ready, isTrue);
      expect(setup.friendReady, isTrue);
      expect(setup.recoveryKey, isNotEmpty);

      final EncryptedChatAttachment photo = await replacement.encryptAttachment(
        bytes: <int>[1, 2, 3],
        name: 'photo.jpg',
        type: 'image/jpeg',
        caption: '',
      );
      expect(photo.envelope, startsWith('cv1:'));
